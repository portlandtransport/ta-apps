...
transitBoard.initializePage = function(data) {	
    ...
    transitBoard.standing_messages.push("<span class=\"tb_misc\">["+data.applianceConfig.id[0]+" "+dimensions+"]</span>");
  }
...
<div id="tb_opaque"></div>\
	...
	var leading_space = "&nbsp;";
	if (data.optionsConfig['stylesheet']) {
		leading_space = ""; // control spacing via css
	}
	// populate initial HTML
	jQuery('#tb_top').html('\
<h1>'+leading_space+'Transit Board&trade; for '+data.optionsConfig.banner[0]+'</h1>\
\
<div id="replace_this">\
<table>\
