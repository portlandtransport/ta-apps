...
function trArrAgencyunitransMakeServiceRequests(stops,options,stop_cache,service_requests) {
                
        if (service_requests.NextBus == undefined) {
                service_requests.NextBus = {};
        }
        if (service_requests.NextBus['unitrans'] == undefined) {
                service_requests.NextBus['unitrans'] = [];
        }
        
        for (var stop_id in stops) {
                var stop_data = stop_cache.stopData('unitrans',stop_id);
                ...
                service_requests.NextBus['unitrans'].push(obj);
	    //console.log(service_requests.NextBus);
        }
