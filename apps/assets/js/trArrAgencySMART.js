...
function trArrAgencySMARTMakeServiceRequests(stops,options,stop_cache,service_requests) {
		
	if (typeof service_requests.Passio == "undefined") {
		service_requests.Passio = {};
	}
	if (typeof service_requests.Passio['SMART'] == "undefined") {
		service_requests.Passio['SMART'] = [];
	}

	for (var stop_id in stops) {
		var stop_data = stop_cache.stopData('SMART',stop_id);
		...
		service_requests.Passio['SMART'].push(obj);
		
	}
	
}

function trArrAgencySMARTEntryFilterCallback(entry) {
	entry.route_data.route_short_name = 'SMART';
	return entry;
}
