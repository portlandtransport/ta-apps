				...
				this.update_aqi = function(retry_count) {
					...
					xhr.onreadystatechange = function() {
						...
						if (xhr.readyState === 4 && xhr.status === 200) {	
							...
							try {
								...
								if ( typeof data != "undefined") {
									...
									if (data.current && data.current.condition) {
										...
										aqi.summary = data.current.condition.text;
										aqi.temperature = Math.floor(data.current.temp_f + 0.5)+"&deg;";
										var icon_url = data.current.condition.icon;
										aqi.icon = '<img src="'+icon_url+'" style="height: 1.4em; margin: -0.2em">';
										aqi.timestamp = new Date();
									} else {
										if (typeof newrelic === "object") {
											newrelic.addPageAction("WEForecast: aqi data issue");
										}
									}
								} else {
									if (typeof newrelic === "object") {
										newrelic.addPageAction("WEJSON: aqi JSON parsing error");
									}									
								}
							} catch (e) {
							...
							if (retry_count >= retry_limit) {
								if (typeof newrelic === "object") {
									newrelic.addPageAction("WE"+xhr.status+": aqi response error "+xhr.statusText,{'errorText': xhr.statusText, 'errorThrown': xhr.status});
								}
							} else {
