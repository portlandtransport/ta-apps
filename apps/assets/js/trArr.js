...
function trArr(input_params) {
	...
	this.draw = function(arrivals_object,displayCallCount,refresh_interval) {
	...
	this.initiate_drawing = function(arrivals_object) {
		...
		if (typeof(arrivals_object.input_params.applicationObject) == "undefined" || arrivals_object.input_params.applicationObject.ready_to_draw) {
			...
			*/
			...
			// initial draw, no delay
			arrivals_object.draw(arrivals_object,displayCallCount,refresh_interval);

			// update quickly over next 10 seconds

			var initiation_interval_id = setInterval(function(){
				arrivals_object.draw(arrivals_object,displayCallCount++,refresh_interval);
			},1000);

			setTimeout(function(){
				// kill fast loop
				clearInterval(initiation_interval_id);

				// set up recurring redraw
				setInterval(function(){
					arrivals_object.draw(arrivals_object,displayCallCount++,refresh_interval);
				},refresh_interval);
			},10*1000);
