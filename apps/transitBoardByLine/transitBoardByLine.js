...
transitBoardByLine.displayPage = function(data, callback) {
	...
	if (transitBoardByLine.weather) {
		if (transitBoardByLine.forecast.weather_is_current()) {
			...
			if (transitBoardByLine.forecast.heatindex != null) {
				summary += ' <span class="heatindex">'+transitBoardByLine.forecast.heatindex+"</span>";
				weather_class = "service_color_weather_heat";
			}
	...
	for (var i = 0; i < arrivalsQueue.length; i++) {
		...
		if (milliseconds_until_arrival >= 0 && milliseconds_until_arrival <= 24*60*60*1000) {
			if (arrivalsQueue[i].stop_data.stop_lat < 45.5852698) {
				...
				if ( arrivalsQueue[i].headsign.match(/downtown vancouver/i) == null || arrivalsQueue[i].headsign.match(/via downtown vancouver/i) != null || !transitBoardByLine.suppress_downtown_only ) {
					tmp_queue.push(arrivalsQueue[i]);
				}
			}
		}
