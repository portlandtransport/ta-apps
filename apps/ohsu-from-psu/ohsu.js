...
ohsu.APP_NAME 		= "CLSB from PSU";
ohsu.APP_VERSION 	= "1.00";
ohsu.APP_ID 			= "clsb-psu";
	...
	*/
	...
	if (typeof arr != "undefined") {
		...
		if (arr[0] != undefined) {
		    ...
		    if (mins <= 0) {
		    	mins = "<b style=\"color: yellow\">Due</b>"
		    } else if (mins < 5) {
		    	mins = "<b style=\"color: yellow\">" +mins + "</b><span style=\"font-size: 30%; color: yellow\"> min</span>";
		    } else {
		    ...
		    //html += '<div class="headsign"><span>' + arr[0].headsign + 
			  //  '</span></div>';
			  html += '<div class="logo" style="float: left"><img src="ohsu.jpg"></div><div class="logo2"  style="float: right"><img height="90" src="psulogo_horiz_wht.png"></div><div style="clear: both"></div>';
			  html += '<div class="headsign" ><span>Streetcar to CLSB<br>from SW Park and Market</span></div>';
		    html += '<div class="mins">' + mins + '</div>';
	
		    if (also.length > 0) {
			    html += '<div class="also"><span>' + also.join(' min, ') + 
			        ' min</span></div>';
		    }
		    else {
		    ...
		    html += '<div class="walking"><span><br>Walk to CLSB '+ohsu.walktime+'</span></div>';
		...
		html = '<div class="box boxeven">';
		 html += '<div class="logo"><img src="ohsu.jpg"></div>';
		html += '<div class="headsign" ><span>Streetcar service begins at 6:30AM on weekdays and 8AM on weekends.</span></div>';
		html += '<div class="walking"><span><br>Walk to CLSB '+ohsu.walktime+'</span></div>';
		html += '</div>';
	}
...
function autoSizeBoxes () {
		...
		// don't resize if iframe is hidden
	  var style = window.getComputedStyle(document.getElementById('boxes'));
    ...
    $('.also').css('font-size', (0.9 * sizes.also) + 'px');
    $('.walking').css('font-size', (0.9 * sizes.walking) + 'px');
    $('.headsign').css('font-size', (0.9 * sizes.headsign) + 'px');
...
$('document').ready( function () {
    trArr({
	    ...
	    initializeCallback: function (data) {
	        ...
	        ohsu.walktime = "15-20 min";
					if (data.optionsConfig != undefined && data.optionsConfig.walktime != undefined && data.optionsConfig.walktime[0] != undefined) {
						ohsu.walktime = data.optionsConfig.walktime[0];
					}
